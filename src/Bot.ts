    ...
    protected async handleWechatMessage(msg: Message, ctx: ContextMessageUpdate) {
        ...
        switch (type) {
            ...
            case MessageType.Image:
                ...
                sent = (image.mimeType || '').toLowerCase().includes('gif') ? await ctx.replyWithSticker({ source: await image.toStream() }) : await ctx.replyWithPhoto({ source: await image.toStream() }, { caption });
                break;
